...
class SimplePageRank(object):
	...
	def update_weights(nodes, num_nodes):
		...
		def distribute_weights((node, (weight, targets))):
			...
			# 1st option: 0.05 to itself. 
			entry = (node, 0.05 * weight)
			resList.append(entry)

			numOfTargets = len(targets)
			# 2nd option: 0.85 randomly follow a link. 
			# Iterate through all targets. 
			if (numOfTargets == 0):
			# Not including current page. 
				percent = 0.85 / (num_nodes - 1) 
				for i in range(num_nodes):
					if i != node:
						resList.append((i, percent * weight))
			else:
				percent = 0.85 / numOfTargets
				for target in targets:
					resList.append((target, percent * weight))

			# 3rd option: 0.10 randomly go to any page. 
			percent = 0.10 

			resList.append((node, percent))


			# Save list of targets as well. 
			resList.append((node, targets))
		...
		def collect_weights((node, values)):
			...
			# Output back into structure of distribute_weight's signature.
			return (node, (newWeight, newTargets))
